...
# x_i = x_test[:, 2:3]
# x_smooth = savgol_filter(x_i.reshape(-1), 51, 3)
...
while idx < 100:
    x_test = group_test.get_group(idx).to_numpy()
    x_i = x_test[:, 2:3]
    x_smooth = savgol_filter(x_i.reshape(-1), 15, 1)

    rmse_smt += np.power((x_smooth[-2] - y_test.to_numpy()[idx - 1]), 2)
    pred.append(x_smooth[-1])
    idx += 1
...
# plt.plot(x_smooth, label='Smooth RUL')
